    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-dark-text mt-1">{isAdmin() ? 'Manage all facility bookings' : 'View and manage your bookings'}</p>
        </div>
        ...
        {filteredBookings.length === 0 ? (
          <div className="bg-dark-card border border-dark-border rounded-xl p-6">
            <EmptyState 
              ...
              action={<button onClick={() => setShowCreateModal(true)} className="mt-2 px-4 py-2 bg-primary hover:bg-primary-dark text-white rounded-lg text-sm font-medium transition-all flex items-center gap-2 mx-auto">
                <HiPlus size={16} /> New Booking
              </button>}
            />
          </div>
        ) : (
          filteredBookings.map((b) => (
            <div key={b.id} className="bg-dark-card border border-dark-border rounded-xl p-5 hover:border-primary/20 transition-all">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-white">{b.facilityName}</h3>
                    <StatusBadge status={b.status} />
                  </div>
                  <p className="text-sm text-dark-text-light mb-2">{b.purpose}</p>
                  <div className="flex flex-wrap items-center gap-4 text-sm text-dark-text">
                    <span className="flex items-center gap-1"><HiCalendar size={14} /> {new Date(b.startTime).toLocaleDateString()}</span>
                    <span className="flex items-center gap-1"><HiClock size={14} /> {new Date(b.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – {new Date(b.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    {isAdmin() && <span className="text-xs">By: {b.userName}</span>}
                  </div>
                  {b.adminReason && (
                    <div className="mt-2 flex items-start gap-1.5 text-sm">
                      <HiChat className="text-dark-text mt-0.5 shrink-0" size={14} />
                ...
                <div className="flex gap-2 shrink-0">
                  {isAdmin() && b.status === 'PENDING' && (
                    <>
                      <button onClick={() => openAction(b, 'approve')} className="flex items-center gap-1 px-3 py-1.5 bg-success/10 text-success border border-success/20 rounded-lg text-sm hover:bg-success/20 transition-colors">
                        <HiCheck size={16} /> Approve
                      </button>
                      <button onClick={() => openAction(b, 'reject')} className="flex items-center gap-1 px-3 py-1.5 bg-danger/10 text-danger border border-danger/20 rounded-lg text-sm hover:bg-danger/20 transition-colors">
                        <HiX size={16} /> Reject
                      </button>
                    </>
                  )}
                  {b.userId === user?.id && (b.status === 'PENDING' || b.status === 'APPROVED') && (
                    <button onClick={() => openAction(b, 'cancel')} className="flex items-center gap-1 px-3 py-1.5 bg-dark-border/50 text-dark-text-light rounded-lg text-sm hover:bg-dark-border transition-colors">
                      <HiBan size={16} /> Cancel
                    </button>
                  )}
                </div>
              </div>
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="New Booking">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-dark-text-light mb-1.5">Start Time</label>
              <input type="datetime-local" required value={form.startTime} onChange={e => setForm({ ...form, startTime: e.target.value })}
                className="w-full px-3 py-2.5 bg-dark border border-dark-border rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
            <div>
              <label className="block text-sm font-medium text-dark-text-light mb-1.5">End Time</label>
              <input type="datetime-local" required value={form.endTime} onChange={e => setForm({ ...form, endTime: e.target.value })}
                className="w-full px-3 py-2.5 bg-dark border border-dark-border rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-dark-text-light mb-1.5">Purpose</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="flex-1 py-2.5 bg-primary hover:bg-primary-dark rounded-lg text-sm font-medium text-white transition-colors shadow-lg shadow-primary/20">Create Booking</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showActionModal} onClose={() => setShowActionModal(false)} title={`${actionType?.charAt(0).toUpperCase() + actionType?.slice(1)} Booking`}>
        <div className="space-y-4">
          ...
          {(actionType === 'approve' || actionType === 'reject') && (
            <div>
              <label className="block text-sm font-medium text-dark-text-light mb-1.5">Reason (optional)</label>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowActionModal(false)} className="flex-1 py-2.5 border border-dark-border rounded-lg text-sm font-medium text-dark-text-light hover:bg-dark-border/40 transition-colors">Cancel</button>
            <button onClick={handleAction}
              className={`flex-1 py-2.5 rounded-lg text-sm font-medium text-white transition-colors shadow-lg
