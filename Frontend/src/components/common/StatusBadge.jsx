export default function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold border transition-colors duration-200 ${styles[status] || 'bg-dark-border/50 text-dark-text-light border-dark-border'}`}>
      {status?.replace(/_/g, ' ')}
    </span>
  );
}
